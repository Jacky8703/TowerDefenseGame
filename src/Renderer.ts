...
export class Renderer {
    ...
    private drawMap(gameState: GameState) {
        ...
        gameState.map.buildableCells.forEach(cell => {
            this.ctx.fillRect(cell.x, cell.y, gameState.map.cellSize, gameState.map.cellSize);
            this.ctx.strokeStyle = 'black';
            this.ctx.lineWidth = 1;
            this.ctx.strokeRect(cell.x, cell.y, gameState.map.cellSize, gameState.map.cellSize);
        });
    }
}